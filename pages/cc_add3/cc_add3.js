...
//交通工具
  ...
  submitTijiao() {
    var that = this
    let dataparams={}
    // console.log(,that.data.renyuanlistarr)
    if(that.data.activeId){
      var peopleId=that.data.activeId 
      var peopleId_is=peopleId.join(",")
    }
    SysConfig.SubSystemData.request({
      istoken: true,
      XKLX: "SYBGGL",
      ...
      data: {
        "XDLMCID": "1001",
        "XDLMSID": "DYBH2020081914363704874819",
        "XDLMA":that.data.eDate,
        "XDLMB":that.data.sDate,
        "XDLMC":peopleId_is
      },
      method: "GET"
    }).then((data) => {
        ...
        if(data.rows.length>0){
          ...
          SysConfig.SubSystemData.request({
            ...
            if(data.rows[0].isbmzr == "1"){  //主任申请，分管院长审核
              return SysConfig.WorkflowManage.create(this.data.onlynum, '出差申请_院长')
            }else if(data.rows[0].Position == "副院长" || data.rows[0].Position == "副所长"){  //副院长申请，院长审核
              return SysConfig.WorkflowManage.create(this.data.onlynum, '出差申请_副院长') 
            }else if(data.rows[0].Position == "院长" || data.rows[0].Position == "所长"){  //院长申请，人事科审核
              return SysConfig.WorkflowManage.create(this.data.onlynum, '出差申请_院长') 
            }else{  //普通申请
            ...
            //       return SysConfig.WorkflowManage.create(this.data.onlynum, '出差申请_主任')  
            //     }else{
            //       return SysConfig.WorkflowManage.create(this.data.onlynum, '出差申请_主任') 
            //     }
