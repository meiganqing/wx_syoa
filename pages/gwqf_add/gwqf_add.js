...
Page({
  ...
  onClickNav({ detail = {} }) {
   ...
   rytoggle(e){
    let tryqrarr_name = "";
    let tryqrarr_=[];
    const {renyuanlistarr}=this.data
    if(e.currentTarget.dataset.ryid==this.data.creatorId){
      wx.showToast({
        title: "请勿选择自己！",
        icon: 'none',
        duration: 1000,
      ...
      if(this.data.rowiddata.rowid){
          if (renyuanlistarr.length>0&&renyuanlistarr[0].rowid) {
            SysConfig.SubSystemData.request({
              istoken: true, //是否使用token
              XKLX: "SYBGGL", //接口XKLX参数
              XAction: "GetDataInterface", //接口XAction参数
              data: { //接口body体内参数
                ...
                "XDLMROWID": renyuanlistarr[0].rowid
              },
              method: "GET"
            })
          }
